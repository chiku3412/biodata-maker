...
export class PreviewDataComponent implements OnInit {
  ...
  constructor(
    @Optional() @Inject(MAT_DIALOG_DATA)public data: { bioDataForm: FormGroup, showAction: boolean, url: any, selectedImage: any },
  ...
  downloadAsPdf() {
    ...
    html2canvas(el, { scale: 2 }).then(canvas => {
      ...
      const fileName = this.bioDataForm.get('fullName')?.value || 'preview';
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      this.isMobileMode = params['mode'] === 'mobile';
    });
  }
}
